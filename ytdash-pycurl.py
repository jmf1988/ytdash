...
def request(url=None, mode='body'):
    ...
    curlobj.setopt(curlobj.TRANSFER_ENCODING, 1)
    curlobj.setopt(curlobj.TCP_KEEPALIVE, 1)
    curlobj.setopt(curlobj.PIPEWAIT, 1)
    curlobj.setopt(curlobj.BUFFERSIZE, 1024)
    # curlobj.setopt(curlobj.KEEP_SENDING_ON_ERROR, 1)
    # curlobj.setopt(curlobj.HTTPHEADER, HEADERS)
    curlobj.setopt(pycurl.FOLLOWLOCATION, 1)
    curlobj.setopt(curlobj.CAINFO, certifi.where())
    if mode == 'head':
        curlobj.setopt(curlobj.NOBODY, 1)
    # elif mode == 'body':
        # content = BytesIO()
    ...
    return curlobj, headers, rawheaders, content


def dict_from_bytes(byteheaders):
    headers = {}
    listheaders = byteheaders.getvalue().decode('iso-8859-1').split('\r\n')
...
def get_media(data):
    baseurl, segmenturl, fd, curltuple, init = data
    curlobj, rawheaders, content = curltuple
    retries503 = 5
    retries40x = 5
    conerr = 0
    twbytes = 0
    ...
    while True:
        try:
            if twbytes:
                ...
                # rheaders[0] = 'Range: bytes=%s-%s' % (sbyte, ebyte)
            gettime = time.time()
            ...
            if init != 1:
                ...
                curlobj.setopt(curlobj.NOBODY, 0)
                curlobj.setopt(curlobj.HEADER, 0)
                curlobj.setopt(curlobj.WRITEDATA, fd)
            ...
            curlobj.setopt(curlobj.HTTPHEADER, rheaders)
...
if __name__ == '__main__':
    ...
    global segsecs, apiurllive, videoid, minvid, lowlatency, manifesturl, curlobj
    ...
    with requests.Session() as session:
        ...
        for urlid in range(len(args.urls)):
            ...
            for sid in range(maxsegms):
                acurlobj, h, arh, acontent = request()
                acurltuple = (acurlobj, arh, acontent)
                vcurlobj, h, vrh, vcontent = request()
                vcurltuple = (vcurlobj, vrh, vcontent)
                segcurlobjs.append((acurltuple, vcurltuple))
            while True:
                starttime = time.time()
                try:
                    sequencenums = []
                    ...
                    for segmresult in segmsresults:
                        ...
                        if not inita == initv == 1 and manifesturl:
                            closefds(rpipes[pid])
                            pid += 1
                    if end:
                        raise Ended
